...
public class SyncHelper {
    ...
    public static void doSync(final Activity activity) {
    ...
    private static class UserIDRunnable implements Runnable {
        ...
        UserIDRunnable(final Account ac, final Context ctx) {
            this.ac = ac;
            this.ctx = ctx;
        }
        ...
        public void run() {
            try {
                UserID = AccountManager.get(ctx).blockingGetAuthToken(ac,
    ...
    public static String getUserId(Context ctx) throws Exception {

        AccountManager am = AccountManager.get(ctx);
        final Account[] ac = am.getAccountsByType(Constants.ACCOUNT_TYPE);
        if (ac.length != 0) {
            UserIDRunnable uid = new UserIDRunnable(ac[0], ctx);
